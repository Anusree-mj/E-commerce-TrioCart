        ...
        if (result.status === 'ok') {
            ...
            orderHelpers.getAllOrderDetails(userId).then((result) => {
                if (result.status === 'ok') {
                    ...
                    res.render('users/orders/orderSuccess', { layout: 'layout/layout', estimatedTym, userName, latestOrder });
                }
            })
        }
        else {
                    ...
                    orderHelpers.getAllOrderDetails(userId).then((result) => {
                        if (result.status === 'ok') {
                            ...
                            productHelpers.getNewArrivalProducts().then(result => {
                                ...
                                res.render('users/orders/orderHistory', { layout: 'layout/layout', user, allCategories, totalCartProduct, orderDetails, viewMoreProducts });
                            })
                        }
                    })
                }
                ...
                if (result) {
                    ...
                    orderHelpers.getAnOrder(orderId).then((result) => {
                        ...
                        productHelpers.getNewArrivalProducts().then(result => {
                            let viewMoreProducts = [
                                ...result.category1, ...result.category2, ...result.category3, ...result.category4];
                            res.render('users/orders/orderDetails', { layout: 'layout/layout', user, allCategories, totalCartProduct, order, viewMoreProducts, userReadableId });
                        })
                    })
                }
            })
